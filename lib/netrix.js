/*
 * netrix.js - Library for making TCP/ICP networking easier and faster to write.
 *
 * Server: Class for creating a TCP/ICP server.
 * Client: Class for joining a TCP/ICP server.
 */
...
function sInit (socket) {
  ...
  socket
    .on('connect', () => {
      log(`${socket.id} connected to ${toArray(this.params).reduce((a, b) => `${a}, ${b}`)}.`)
...
class Server extends net.Server {
  constructor () {
    ...
    this
      .on('listening', () => {
        log(`Hub listening to ${toArray(this.params).reduce((a, b) => `${a}, ${b}`)}.`)
      })
